...
# Lazy TF/Keras - define class inside loader to avoid top-level import issues
print("[INFO] App loaded - model loads on first request")
    ...
    with _model_lock:
        ...
        try:
            ...
            try:
                ...
                pass
            # Try without custom objects first (common fix)
            model_obj = tf.keras.models.load_model(MODEL_PATH, compile=False)
            model = model_obj
            print("[OK] Model loaded successfully with tf.keras (no custom)")
            return True, None
            ...
            try:
                ...
                model = model_obj
                print("[OK] Model loaded successfully with standalone keras (no custom)")
                return True, None
